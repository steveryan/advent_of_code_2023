data = File.read("data/day_1.txt").lines.map { |line| line.strip }
...
def build_calibration_lines(data)
  calibration_lines = data.map do |line|
    ...
    while first_num.nil? || last_num.nil?
      unless first_num
        first_num = line[index] if line[index].number?
      end
      reverse_index = line.chars.size - index - 1
      unless last_num
        last_num = line[reverse_index] if line[reverse_index].number?
      end
      index += 1
    end
    combined_string = first_num.to_s + last_num.to_s
